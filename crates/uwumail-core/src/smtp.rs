...
fn mailbox(address: &Address) -> Result<Mailbox> {
    ...
    Ok(Mailbox::new(address.name.clone().filter(|n| !n.trim().is_empty()), email))
}
...
fn attachment_body(attachment: &OutgoingAttachment) -> Result<Vec<u8>> {
    match &attachment.source {
        ...
        AttachmentSource::Path { path } => {
            std::fs::read(path).map_err(|e| Error::invalid(format!("Couldn't read {}: {e}", attachment.filename)))
        }
    }
}
...
pub fn build(
    ...
    if let Some(threading) = threading {
        let parent = format!("<{}>", threading.parent_message_id);
    ...
    let message = if attachments.is_empty() {
        ...
        for attachment in attachments {
            ...
            let part: SinglePart =
                Attachment::new(attachment.filename.clone()).body(attachment_body(attachment)?, content_type);
